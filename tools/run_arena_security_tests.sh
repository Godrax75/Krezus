#!/usr/bin/env bash
# Tests de 0020 — durcissement des fonctions, recherche de joueurs,
# valeurs quotidiennes et classement par période — sur un Postgres jetable.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
DB="krezus_arena_security_test_$$"
...
for m in 0006_oracle 0007_arena 0008_hercule 0009_notifications 0015_paper_identity_guard \
         0017_portfolio_history 0019_referrals 0020_arena_search_leaderboard; do
  run "$ROOT/supabase/migrations/$m.sql"
done

echo "→ exécution des tests"
psql -v ON_ERROR_STOP=1 -d "$DB" -f "$ROOT/supabase/tests/arena_security_test.sql" 2>&1 \
